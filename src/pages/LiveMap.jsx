...
// );
// -- Keep only latest location per user (optional cleanup):
// create unique index staff_locations_user_latest on staff_locations (user_id, recorded_at desc);

import { useEffect, useState, useRef, useCallback } from 'react'
import {
  Navigation, MapPin, RefreshCw, Radio,
  RadioTower, X, Users, Clock, Eye, EyeOff,
...
export default function LiveMap() {
  ...
  const [locations, setLocations]     = useState([])   // all staff latest locs
  const [loading, setLoading]         = useState(true)
  const [intelCountry, setIntelCountry] = useState(null)
  const watchRef                       = useRef(null)
  const updateRef                      = useRef(null)  // debounce DB writes
  ...
  const loadData = async () => {
    ...
    const [{ data: prof }, { data: trip }, { data: locs }] = await Promise.all([
      ...
      supabase.from('itineraries').select('*')
        ...
        .limit(1).single(),
      // Latest location per user (last 24h)
    ...
    const seen = new Set()
    const dedupLocs = (locs || []).filter(l => {
      if (seen.has(l.user_id)) return false
      seen.add(l.user_id)
      return true
    })
    setLocations(dedupLocs)
    setLoading(false)
  }
  ...
  const sharingCount = locations.filter(l => l.user_id !== profile?.id).length
  const defaultCenter = myPos
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-[12px] border border-gray-200 overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.08)]"
            ...
            <MapContainer
              ...
              {myPos && (
                <>
                  <MapRecenter lat={myPos.latitude} lon={myPos.longitude}/>
                  <Marker
                    position={[myPos.latitude, myPos.longitude]}
                    icon={MyIcon(BRAND_GREEN)}>
                    <Popup>
                      <div className="text-xs">
                        <p className="font-bold">{profile?.full_name || 'You'} (live)</p>
                        <p className="text-gray-500">±{Math.round(myPos.accuracy || 0)}m accuracy</p>
                        {activeTrip && <p className="mt-1 font-medium">{activeTrip.trip_name}</p>}
                      </div>
                    </Popup>
                  </Marker>
                </>
              ...
              {/* Other staff locations */}
              {locations.filter(l => l.user_id !== profile?.id).map(loc => {
                ...
                return (
                  <Marker
                    ...
                    icon={makeIcon(BRAND_BLUE, inits)}>
                    <Popup>
                      <div className="text-xs space-y-1">
                        <p className="font-bold text-sm">{loc.full_name}</p>
                        {loc.trip_name && <p className="text-gray-600">{loc.trip_name}</p>}
                        {loc.arrival_city && <p className="flex items-center gap-1 text-gray-500"><MapPin size={9}/>{loc.arrival_city}</p>}
                        <p className="text-gray-400">Updated {timeAgo(loc.recorded_at)}</p>
                        {country && (
                          <button onClick={() => setIntelCountry(country)}
                            className="text-[#0118A1] hover:underline font-medium block mt-1">
                            {country} intel brief →
                          </button>
                        )}
                      </div>
            ...
            {loading ? (
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto">
                {locations.map(loc => {
                  ...
                  return (
                    <div key={loc.id} className={`flex items-center gap-2.5 p-2.5 rounded-[8px] border ${isMe ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
