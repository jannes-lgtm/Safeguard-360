...
import { comprehensiveRiskScan, fetchGDACS, fetchUSGS, fetchHealthOutbreaks, fetchWeatherAlerts, fetchHurricaneAlerts } from './_claudeSynth.js'
...
export const scanCountry = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const feeds = await step.run('fetch-feeds', async () => {
      ...
      const [gdacsEvents, quakes, health, weatherAlerts, hurricaneAlerts] = await Promise.all([
        ...
        fetchHurricaneAlerts(country).catch(() => []),
      ])
      ...
      return { gdacsEvents, quakes, health, weatherAlerts, hurricaneAlerts, internalAlerts }
    })
    ...
    const allRows = await step.run('build-rows', async () => {
      ...
      for (const trip of trips) {
        ...
        for (const ha of feeds.hurricaneAlerts || []) {
          rows.push({
            itinerary_id: trip.id, user_id: trip.user_id,
            alert_type:   'weather', severity: ha.severity,
            title:        ha.title,
            description:  ha.description || null,
            source:       ha.source, source_url: ha.source_url,
            country, arrival_city: trip.arrival_city, trip_name: trip.trip_name,
            dedup_key:    `nhc-${ha.title.toLowerCase().replace(/\s+/g,'-').slice(0,50)}-${trip.id}-${today}`,
