...
function SlidePanel({ staff, countryRisk, onClose, onOpenIntel }) {
  ...
  return (
    <div className="fixed inset-0 flex" style={{ zIndex: 9000 }}>
      ...
      <div className="w-full max-w-md flex flex-col h-full overflow-y-auto" style={{ background: DS.surface, borderLeft: `1px solid ${DS.border}` }}>
        <div className="flex items-center justify-between px-5 py-4 sticky top-0" style={{ borderBottom: `1px solid ${DS.border}`, background: DS.bgAlt }}>
        ...
        <div className="p-5 flex flex-col gap-5">
          ...
          {trip ? (
            <>
              ...
              {country && risk && (
                <div>
                  ...
                  <button onClick={() => { onClose(); onOpenIntel(country, staff.full_name || staff.email, trip.return_date, trip.arrival_city) }}
                    className="mt-2 flex items-center gap-1.5 text-xs font-semibold text-[#AACC00] hover:underline">
              ...
              {/* Trip details */}
              <div className="rounded-[8px] p-4" style={{ background: DS.bgAlt, border: `1px solid ${DS.border}` }}>
...
function CheckInCard({ ci }) {
  ...
  const badge  = ci.status === 'distress'
    ? <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0" style={{ background: 'rgba(138,46,46,0.12)', border: '1px solid rgba(138,46,46,0.30)', color: '#EF7474' }}><AlertCircle size={9}/>DISTRESS</span>
    : <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0" style={{ background: 'rgba(170,204,0,0.10)', border: '1px solid rgba(170,204,0,0.25)', color: '#AACC00' }}><CheckCircle2 size={9}/>Safe</span>

  return (
    <div className="last:border-0"
      style={{ borderBottom: `1px solid ${DS.border}`, background: ci.status === 'distress' ? 'rgba(138,46,46,0.08)' : 'transparent' }}>

      {/* ── Collapsed row ── */}
      <button
        className="w-full px-4 sm:px-5 py-3 text-left transition-colors"
        onClick={() => setOpen(p => !p)}
      >
        {/* Mobile layout */}
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          <span className={`text-xs shrink-0 w-28 text-right font-medium ${isOverdue ? 'text-[#EF7474]' : 'text-gray-400'}`}>
      ...
      {open && (() => {
        ...
        return (
          <div className="px-5 pb-5 pt-3" style={{ borderTop: `1px solid ${DS.border}`, background: DS.bgAlt }}>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <p className={`text-xs font-medium ${isOverdue ? 'text-[#EF7474]' : 'text-gray-700'}`}>
                      {ci.next_checkin_due ? fmtDue(ci.next_checkin_due) : '—'}
                      {isOverdue && <span className="ml-1 text-[9px] px-1.5 py-0.5 rounded-full" style={{ background: 'rgba(138,46,46,0.14)', color: '#EF7474' }}>OVERDUE</span>}
                    </p>
                  </div>
                </div>
                {hasLocation && (
    ...
    <Layout>
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <button onClick={load} disabled={loading}
          className="inline-flex items-center gap-1.5 text-sm text-[#AACC00] font-medium hover:underline disabled:opacity-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <MetricCard label="Currently Travelling" value={loading ? '–' : travelling}            valueColor="text-[#6EA8C8]"  icon={Users} />
        <MetricCard label="Total Staff" value={loading ? '–' : staffList.length} valueColor="text-[#D0D4DC]" icon={Users} />
        <MetricCard label="Active Alerts"         value={loading ? '–' : activeAlertCount}     valueColor="text-[#D4A64A]"  icon={Bell}  />
      </div>

      {/* Staff list — mobile cards + desktop table */}
      <div className="rounded-[8px] overflow-hidden" style={{ background: DS.surface, border: `1px solid ${DS.border}` }}>
        ...
        <div className="block sm:hidden divide-y divide-gray-100">
          {loading ? (
            ...
            return (
              <div key={staff.id}
                className={`px-4 py-3.5 ${staff.trip ? 'bg-[rgba(58,88,112,0.06)]' : ''}`}>
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button onClick={() => setSelectedStaff(staff)}
                    className="shrink-0 text-xs font-semibold rounded-[4px] px-3 py-1.5 transition-colors"
                    style={{ color: DS.green, border: `1px solid rgba(170,204,0,0.25)`, background: 'transparent' }}>
                    View
                  </button>
                </div>
                    ...
                    <div className="flex flex-wrap items-center gap-1 text-xs text-gray-500">
                      ...
                      {country && (
                        <button
                          ...
                          className="text-[#AACC00] font-medium hover:underline ml-0.5">
                          · {country} intel →
                        </button>
                      )}
                    </div>
                    <div className="flex items-center gap-3 text-xs text-gray-400">
                      {ciLabel && (
                        <span className="flex items-center gap-1">
                          <CheckCircle2 size={10} className="text-[#AACC00]"/>
                          {ciLabel}
                        </span>
                      )}
        ...
        <table className="hidden sm:table w-full text-sm">
          <thead>
            <tr style={{ borderBottom: `1px solid ${DS.border}`, background: DS.bgAlt }}>
            ...
            {loading ? (
              ...
              return (
                <tr key={staff.id}
                  ...
                  style={{ borderBottom: `1px solid ${DS.border}`, background: staff.trip ? 'rgba(58,88,112,0.06)' : 'transparent' }}>
                    ...
                    {staff.trip?.arrival_city ? (
                      <div>
                        ...
                        {country && (
                          <button
                            onClick={() => openIntel(country, staff.full_name || staff.email, staff.trip.return_date, staff.trip.arrival_city)}
                            className="text-[10px] text-[#AACC00] hover:underline flex items-center gap-0.5 font-medium mt-0.5">
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="flex items-center gap-1.5">
                          <CheckCircle2 size={11} className="text-[#AACC00] shrink-0" />
      ...
      <div className="mt-6 rounded-[8px] overflow-hidden" style={{ background: DS.surface, border: `1px solid ${DS.border}` }}>
        <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: `1px solid ${DS.border}`, background: DS.bgAlt }}>
          <div className="flex items-center gap-2">
            <CheckCircle2 size={14} className="text-[#AACC00]" />
        ...
        {loading ? (
          ...
          <div>
            ...
            <div className="hidden sm:flex items-center gap-3 px-5 py-2" style={{ borderBottom: `1px solid ${DS.border}`, background: DS.surface }}>
