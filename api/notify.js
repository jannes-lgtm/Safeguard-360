  ...
  return `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
      <div style="background:#1E2461;padding:20px 24px;">
        ...
        <p style="color:#a5b4fc;margin:4px 0 0;font-size:13px;">Travel Risk & Duty of Care</p>
      </div>
      <div style="padding:24px;">
        ...
        <p style="margin:20px 0 0;font-size:13px;color:#6b7280;">This alert was sent to ${travelerName} and their emergency contacts by SafeGuard360.</p>
      </div>
    </div>
  `
}
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
      ...
      <div style="padding:24px;">
        <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:6px;padding:16px;margin-bottom:20px;">
          <p style="margin:0;font-size:16px;font-weight:bold;color:${color};">${icon} ${country} — ${severity} Risk</p>
        </div>
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
          <tr><td style="padding:8px 0;color:#6b7280;width:140px;">Trip</td><td style="padding:8px 0;font-weight:600;">${tripName || '—'}</td></tr>
        ...
        ${sources?.length ? `
          <div style="margin-top:16px;">
            ...
            ${sources.map(s => `<a href="${s.url}" style="display:block;font-size:13px;color:#1E2461;margin-bottom:4px;">${s.name} →</a>`).join('')}
          </div>
        ` : ''}
        <p style="margin:20px 0 0;font-size:13px;color:#6b7280;">This alert was sent to ${travelerName} and their emergency contacts by SafeGuard360.</p>
      </div>
...
export default async function handler(req, res) {
  ...
  const { type, recipients, data } = JSON.parse(body)
  ...
  if (type === 'flight') {
    ...
    subject = `🚨 Risk Alert: ${data.country} — ${data.severity} | ${data.tripName || 'Your trip'}`
  ...
  try {
    ...
    if (!r.ok) return res.status(r.status).json({ error: result.message || 'Send failed' })
